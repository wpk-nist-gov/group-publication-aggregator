...
nb_kernel_rgx_aliases = {
    "group-publication-aggregator.*": "python3",
    "conda.*": "python3",
}
...
html_context = {
    ...
    "github_repo": "group-publication-aggregator",
    "github_version": "main",
    "doc_path": "docs",
}
...
project = "group-publication-aggregator"
...
def _get_version() -> str:
    if (version := os.environ.get("SETUPTOOLS_SCM_PRETEND_VERSION")) is None:
        version = group_publication_aggregator.__version__
    return version
...
html_theme_options = {
    ...
    # "canonical_url": "",
    "repository_url": f"https://github.com/{github_username}/group-publication-aggregator",
...
htmlhelp_basename = "group_publication_aggregatordoc"
...
latex_documents = [
    (
        master_doc,
        "group_publication_aggregator.tex",
        "group-publication-aggregator Documentation",
...
def linkcode_resolve(domain: str, info: dict[str, Any]) -> str | None:
    ...
    # fmt: off
    fn = os.path.relpath(fn, start=Path(group_publication_aggregator.__file__).parent)
    # fmt: on

    return f"https://github.com/{github_username}/group-publication-aggregator/blob/{html_context['github_version']}/src/group_publication_aggregator/{fn}{linespec}"
