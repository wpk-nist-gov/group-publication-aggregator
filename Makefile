...
version-import: ## check version from python import
	-$(UVRUN) python -c 'import group_publication_aggregator; print(group_publication_aggregator.__version__)'
...
install-kernel:  ## install kernel
    ...
    --display-name "Python [venv: group-publication-aggregator-dev]"
...
tuna-import	: ## Analyze load time for module
	$(UVRUN) python -X importtime -c 'import group_publication_aggregator' 2> tuna-loadtime.log
